...
class GeoCache:
    ...
    def get(self, ip: str) -> Optional[HopGeo]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                "SELECT 1 FROM geo_cache WHERE ip = ? AND expires_at <= ?",
                (ip, now)
            )
            if cursor.fetchone():
    ...
    def set(
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                """,
                (
                    ip, geo.lat, geo.lon, geo.city, geo.country, geo.country_code,
                    geo.asn, geo.asn_org, source, confidence, now, expires_at
                )
            )
    ...
    def get_stats(self) -> dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                "SELECT COUNT(*) FROM geo_cache WHERE expires_at > ?",
                (int(time.time()),)
            )
...
class ASNCache:
    ...
    def get(self, ip: str) -> Optional[tuple[int, str]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT asn, asn_org FROM asn_cache WHERE ip = ? AND expires_at > ?",
                (ip, now)
            )
            row = cursor.fetchone()
...
class DNSCache:
    ...
    def get(self, ip: str) -> Optional[str]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT hostname FROM dns_cache WHERE ip = ? AND expires_at > ?",
                (ip, now)
            )
            row = cursor.fetchone()
